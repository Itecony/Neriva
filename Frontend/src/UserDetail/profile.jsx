...
export default function Profile({ userId = null, isPersonal = true }) {
  ...
  const checkMentorStatus = async (targetId) => {
    try {
      const token = localStorage.getItem('authToken');
      const res = await fetch(`${API_BASE_URL}/api/mentors/${targetId}`, {
  ...
  const checkFollowStatus = async (targetId) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/users/${targetId}/is-following`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      if (response.ok) {
  ...
  const fetchFollowStats = async (targetId) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(
        `${API_BASE_URL}/api/users/${targetId}/follower-count`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      if (response.ok) {
  ...
  const handleShowStats = async (type) => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(
        `${API_BASE_URL}/api/users/${targetId}/${type}?limit=100`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      if (response.ok) {
        const result = await response.json();
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/users/${targetId}/follow`, {
        method: method,
  ...
  const fetchPosts = async (profileId) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(
        `${API_BASE_URL}/api/posts?userId=${profileId}`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
          ...
          <div className="px-6 pb-6">
            <div className="flex flex-col items-center md:flex-row md:items-end md:justify-between gap-4 -mt-12 md:-mt-16 mb-6">

              {/* ✅ Profile Image or Initials */}
              <div className="flex flex-col items-center md:flex-row md:items-end gap-4 text-center md:text-left">
                {profileAvatarUrl ? (
                  <img
                    src={profileAvatarUrl}
                    alt={profile.firstName}
                    className="w-24 h-24 md:w-32 md:h-32 rounded-xl border-4 border-white shadow-lg object-cover bg-white"
                  />
                ) : (
              ...
              <div className="flex flex-wrap justify-center md:justify-end gap-3 w-full md:w-auto">
                {isPersonal ? (
                  <>
                    ...
                    <button onClick={() => setShowEditProfileModal(true)} className="flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors" title="Edit profile">
            ...
            <div>
              ...
              {userInterests.length > 0 ? (
                ...
                <div className="bg-gray-50 rounded-lg p-3 text-center">
                  {isPersonal ? (
                    <button onClick={handleAddInterest} className="mt-2 text-teal-600 hover:text-teal-700 text-sm font-medium">Add your interests</button>
                  ) : (
      ...
      {showEditProfileModal && <EditProfileModal isOpen={showEditProfileModal} onClose={() => setShowEditProfileModal(false)} currentUser={profile} onUpdate={handleProfileUpdate} />}
    </div>
  );
}
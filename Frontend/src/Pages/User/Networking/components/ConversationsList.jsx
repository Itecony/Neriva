...
import { Search, MessageCircle, UserPlus, Users, MoreVertical, ArrowRight } from 'lucide-react';
import socketService from '../../../../utils/socketService';
import { API_BASE_URL } from '../../../../config';
import { useNotifications } from '../../../../context/NotificationContext'; // ✅ Import Context

export default function ConversationsList({ onSelectConversation, selectedConversationId, currentUserId }) {
  ...
  const { clearAlert } = useNotifications(); // ✅ Use Context

  // ✅ NEW: Clear alerts when this component mounts (i.e., user views networking)
  useEffect(() => {
    clearAlert('networking');
  }, []);
  ...
  useEffect(() => {
    const handleListUpdate = (data) => {
      ...
      setConversations(prev => {
        ...
        const index = prev.findIndex(c => c.id === msgConversationId);
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      const [followingRes, followersRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/users/${targetUserId}/following?limit=100`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/users/${targetUserId}/followers?limit=100`, { headers: { 'Authorization': `Bearer ${token}` } })
      ]);
  ...
  return (
    <div className="flex flex-col h-full w-full lg:w-80 bg-white border-r border-gray-200 shadow-sm">
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="hidden lg:block p-4 bg-teal-600 text-white">
          ...
          <div className="divide-y divide-gray-50">
            {filteredList.map((item) => {
              if (activeTab === 'chats') {
                ...
                // ✅ MATCH: Use logic from ChatSpace
                const otherUser = item.participants?.find(p => (p.id || p._id) !== currentUserId) || item.participants?.[0];

                // const getImageUrl moved to top
                const displayName = isGroup
                  ? item.name
                  : (otherUser?.firstName
                    ? `${otherUser.firstName} ${otherUser.lastName || ''}`.trim()
                    : (otherUser?.username || 'Unknown User'));
                const rawImage = isGroup ? null : (otherUser?.avatar || otherUser?.profileImage || otherUser?.profilePicture);
                ...
                return (
                  <button key={item.id} onClick={() => handleChatClick(item)} className={`w-full flex items-center gap-3 p-4 hover:bg-gray-50 transition-colors text-left ${isSelected ? 'bg-teal-50 border-r-4 border-teal-500' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`text-xs truncate h-5 flex items-center ${isTyping ? 'text-teal-600 font-medium' : 'text-gray-500'}`}>
                        {isTyping ? (
                ...
                const rawImage = contact.profilePicture || contact.profileImage || contact.avatar;
                const image = getImageUrl(rawImage);
                return (

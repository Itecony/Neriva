...
export default function UserProfileModal({ user, isOpen, onClose, isGroup = false }) {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const followRes = await fetch(
        `${API_BASE_URL}/api/users/${userId}/follower-count`,
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl w-[90%] sm:w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden"
        ...
        <div className="p-4 sm:p-6 space-y-3 sm:space-y-5 max-h-[70vh] overflow-y-auto">
          {!isGroup ? (
            <>
                ...
                <div className="relative mb-2 sm:mb-3">
                  {user.profileImage || user.profilePicture ? (
                    <img
                      ...
                      alt={user.firstName}
                      className="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover border-4 border-gray-50 shadow-sm"
                    />
                  ) : (
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-teal-400 to-cyan-600 flex items-center justify-center text-white text-xl sm:text-2xl font-bold shadow-sm">
                      {getInitials()}
                    </div>
                  )}
                </div>
              ...
              {/* ✅ LIVE STATS GRID */}
              <div className="grid grid-cols-3 gap-2 py-2 sm:py-3 border-y border-gray-100">

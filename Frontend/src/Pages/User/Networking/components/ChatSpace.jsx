...
export default function ChatSpace({ conversationId, conversation, onBack, currentUserId }) {
  ...
  // ✅ FIX: Use prop to match Networking and List logic (handles Number/String correctly)
  ...
  useEffect(() => {
    ...
    const handleRealTimeMessage = (data) => {
      ...
      const msgConversationId = incomingMsg.conversationId || incomingMsg.conversation_id;
  ...
  const handleSendMessage = async (e) => {
    ...
    const safeContent = sanitizeText(newMessage); // ✅ Sanitize
    if (!safeContent) return; // Guard against empty after sanitization

    // Optimistic Update
    const tempId = Date.now();
    const tempMsg = {
      id: tempId,
      content: safeContent, // Use safe content
  ...
  const rawImage = displayUser?.profileImage || displayUser?.profilePicture || displayUser?.avatar;
  const profileImage = rawImage ? (rawImage.startsWith('http') ? rawImage : `${API_BASE_URL}/${rawImage}`) : null;
  ...
  return (
    <div className="flex flex-col h-full bg-white lg:rounded-tr-xl lg:rounded-br-xl shadow-lg overflow-hidden">
      <div className="sticky top-0 z-10 flex items-center justify-between p-2 sm:p-4 border-b border-gray-100 bg-white">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setShowProfile(true)}>
            {profileImage ? (
              <img src={profileImage} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover" alt={displayName} />
            ) : (
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-teal-100 flex items-center justify-center text-teal-600 font-bold text-sm">
                {isGroup ? <Users className="w-5 h-5" /> : getInitials()}
              </div>
            )}
            <div>
              <h2 className="font-bold text-gray-900 leading-tight text-sm sm:text-base">{displayName}</h2>
        ...
        <div className="flex items-center gap-1">
          {/* <button className="p-1.5 sm:p-2 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-full transition-all"><Phone className="w-4 h-4 sm:w-5 sm:h-5" /></button>
          <button className="p-1.5 sm:p-2 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-full transition-all"><Video className="w-4 h-4 sm:w-5 sm:h-5" /></button> */}
          <button onClick={() => setShowProfile(true)} className="p-1.5 sm:p-2 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-full transition-all"><MoreVertical className="w-4 h-4 sm:w-5 sm:h-5" /></button>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-2 sm:space-y-4 bg-gray-50/50">
        {loading ? (
      ...
      <form onSubmit={handleSendMessage} className="p-2 sm:p-4 bg-white border-t border-gray-100">
        <div className="flex gap-2 bg-gray-50 p-1.5 sm:p-2 rounded-xl border border-gray-200 focus-within:border-teal-500 focus-within:ring-2 focus-within:ring-teal-100 transition-all">
          <input
            type="text"
            value={newMessage}
            onChange={handleInputChange}
            placeholder="Type your message..."
            className="flex-1 bg-transparent px-2 py-1.5 sm:px-3 sm:py-2 outline-none text-gray-700 placeholder-gray-400"
          />

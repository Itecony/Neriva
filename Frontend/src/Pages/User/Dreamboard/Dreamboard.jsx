...
function PostCard({ post, onClick }) {
  ...
  const authorImage = author.avatar || author.profileImage;
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer border border-gray-100" onClick={onClick}>
        ...
        <div className="flex items-center gap-3 group" onClick={handleProfileClick}>
          {authorImageUrl ? (
            <img
              src={authorImageUrl}
              alt={authorName}
              className="w-10 h-10 rounded-full object-cover shadow-sm group-hover:scale-105 transition-all"
            />
          ) : (
  ...
  return (
    <div className="flex gap-3 pb-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 p-2 rounded-lg transition-colors cursor-pointer" onClick={onClick}>
      <div className="flex flex-col items-center gap-1 text-gray-600 text-xs min-w-[50px]">
        <div className="flex items-center gap-1"><MessageCircle className="w-3 h-3" /><span>{Array.isArray(idea.comments) ? idea.comments.length : (idea.comments || 0)}</span></div>
        <div className="flex items-center gap-1"><Eye className="w-3 h-3" /><span>{idea.views || 0}</span></div>
      </div>
      <div className="flex-1">
        <h4 className="text-sm font-medium text-blue-600 mb-1 hover:underline line-clamp-1">{idea.title}</h4>
        <p className="text-xs text-gray-600 line-clamp-2">{idea.content?.replace(/<[^>]*>/g, '') || idea.description}</p>
      </div>
    </div>
  );
}
  ...
  <div className="space-y-4">
    <div className="flex p-1 bg-gray-100 rounded-xl mb-4 relative">
      <button
        ...
        className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${activeTab === 'trending' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'
          }`}
      >
      ...
      <button
        ...
        className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${activeTab === 'ideas' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'
          }`}
      >
    ...
    {activeTab === 'trending' ? (
      ...
      ideas && ideas.length > 0 ? (
        ideas.map((idea, index) => (
          <IdeaCard key={idea.id || index} idea={idea} onClick={() => onPostClick(idea)} />
        ))
      ) : (
        <div className="text-center py-8 text-gray-400 text-sm">
          <p>No ideas found matching your interests.</p>
          <p className="text-xs mt-2">Update your profile interests to see relevant ideas!</p>
        </div>
      )
    )}
...
export default function Dreamboard() {
  ...
  const [interestPosts, setInterestPosts] = useState([]); // NEW: Posts matching interests
  ...
  const fetchInterestPosts = async (interests) => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      // Fetch posts matching interests (limit 5 for sidebar)
      const response = await fetch(`${API_BASE_URL}/api/posts?interests=${interests}&limit=5`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (profileData.interests && profileData.interests.length > 0) {
          fetchInterestPosts(profileData.interests);
        }
      }
    } catch (err) {
  ...
  const fetchTopPosts = async () => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/api/posts/top?limit=5`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  // ✅ NEW: Handle updates from Modal (Likes/Comments)
  const handlePostUpdate = (updatedPost) => {
    // Update main feed
    setPosts(prev => prev.map(p => p.id === updatedPost.id ? { ...p, ...updatedPost } : p));

    // Update top posts if applicable
    setTopPosts(prev => prev.map(p => p.id === updatedPost.id ? { ...p, ...updatedPost } : p));

    // Update selected post state if it's the one being viewed (to keep modal in sync effectively or redundant but safe)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Modals */}
      {selectedPost && <PostModal post={selectedPost} mode="view" onClose={() => setSelectedPost(null)} onSave={handlePostSaved} onUpdate={handlePostUpdate} />}
